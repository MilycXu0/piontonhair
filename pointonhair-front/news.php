...
//登出
      if(isset($_POST["submit"]) && $_POST["submit"] == "登出"){
        ...
        echo '<script>alert("注销成功");window.open("news.php","_self:")</script>';
        exit;
      }
//注册
...
<?php
$nn=@$_GET['id'];
$sql="select * from news where mid=".$nn;
$result=mysql_query($sql) or die(mysql_error());
...
<div class="main contain">
  <div class="news">
    ...
    <span><i><?php echo $row['newsdate'] ?></i>&nbsp;<?php echo $row['newsauthor'] ?></span>
    <div class="clear"></div>
    <div class="content"><?php echo $row['newscontent']?></div>
  </div>
</div>
